...
export default function Incomes() {
  ...
  const queryParams = useMemo(() => ({ search, currency }), [currency, search]);
  const { items, pagination, loading, error, setPage, setPageSize } =
  ...
  const removeIncome = async (income: ServiceSaleType) => {
    if (!window.confirm(`¿Eliminar el ingreso “${income.serviceName}”?`))
      return;
    try {
  ...
  const columns: readonly Column<ServiceSaleType>[] = [
    ...
    { key: "description", label: "Descripción", width: "35%", truncate: true },
    {
      ...
      align: "right",
      render: (income) =>
        `${SYMBOLS[income.currency]} ${Number(income.amount).toLocaleString(
          "es-PE",
          {
    ...
    { key: "currency", label: "Moneda", align: "center" },
    {
      ...
      render: (income) => (
        <div className="flex justify-center gap-2">
          <Permission permission="incomes.manage">
            <button
              type="button"
          ...
          <Permission permission="incomes.delete">
            <button
              type="button"
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Registra ingresos adicionales que formarán parte del resumen
