...
export default function TableViewPO({
  purchaseOrder,
  seeSalesPrices,
}: TableViewPOProps) {
  const { can } = useAccess();
  ...
  const currencySymbol =
    purchaseOrder?.supplier?.currency.toUpperCase() === "PEN" ? "S/." : "$";
  ...
  return (
    <div
      className={`overflow-x-auto ${can("finance.view") ? "" : "order-items-no-finance"}`}
    >
      <table className="text-center w-full">
        <thead className="bg-[#14519d] border-1 border-[#14519d] text-white">
          <tr>
            ...
            <th className="p-2 border-r-1 border-[#f3f4f6] text-nowrap">
              DESCRIPCIÓN
            </th>
        ...
        <tbody className="border-1 border-gray-400">
          {orderedResources.map((item, index) => (
            <tr key={index}>
              ...
              <td className="p-2 border-1 border-gray-400 text-nowrap">
                {currencySymbol}
                {formatPrice(item.unitPurchasePrice, 6)}
              </td>
              <td className="p-2 border-1 border-gray-400 bg-gray-100 text-nowrap">
                {currencySymbol}
                {formatPrice(
                  lineAmount(item.quantity, item.unitPurchasePrice),
                  2,
                )}
              </td>
              {seeSalesPrices && (
                <td className="p-2 border-1 border-gray-400 text-nowrap">
                  {currencySymbol}
                  {formatPrice(item.unitSalesPrice, 2)}
                </td>
              )}
              {seeSalesPrices && (
                <td className="p-2 border-1 border-gray-400 bg-gray-100 text-nowrap">
                  {currencySymbol}
                  {formatPrice(
                    lineAmount(item.quantity, item.unitSalesPrice),
                    2,
                  )}
                </td>
              )}
        ...
        <tfoot>
          <tr>
            <td
              colSpan={seeSalesPrices ? colSpan(5) : colSpan(7)}
              className="p-2 pr-8 font-bold text-right table-cell"
            >
              SUBTOTAL
            </td>
            <td className="p-2 border-1 border-gray-400 bg-gray-100">
              {currencySymbol} {formatPrice(subtotalCompra, 2)}
            </td>
            {seeSalesPrices && <td></td>}
            {seeSalesPrices && (
              <td className="p-2 border-1 border-gray-400 bg-gray-100">
                {currencySymbol} {formatPrice(subtotalVenta, 2)}
              </td>
            )}
          </tr>
          <tr>
            ...
            <td className="p-2 border-1 border-gray-400 bg-gray-100">
              {currencySymbol} {formatPrice(igvCompra, 2)}
            </td>
            {seeSalesPrices && <td></td>}
            {seeSalesPrices && (
              <td className="p-2 border-1 border-gray-400 bg-gray-100">
                {currencySymbol} {formatPrice(igvVenta, 2)}
              </td>
            )}
          </tr>
          <tr>
            ...
            <td className="p-2 border-1 border-gray-400 bg-gray-800 text-white">
              {currencySymbol} {formatPrice(totalCompra, 2)}
            </td>
            {seeSalesPrices && <td></td>}
            {seeSalesPrices && (
              <td className="p-2 border-1 border-gray-400 text-white bg-gray-800">
                {currencySymbol} {formatPrice(totalVenta, 2)}
              </td>
            )}
          </tr>
        </tfoot>
