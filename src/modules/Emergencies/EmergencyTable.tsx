...
export default function EmergencyTable() {
      ...
      render: (row: EmergencyType) => {
        return (
          <span
            ...
            style={{
              backgroundColor:
                bgStatusColor[row.status as keyof typeof bgStatusColor] ||
                "#9ca3af",
            }}
          >
    ...
    },
    {
      ...
      render: (row: EmergencyType) => {
        return (
          <SeeButton
            onClick={() =>
              navigate(`/admin/emergencies/${row.emergencyId}`, {
  ...
  return (
    <div className="flex flex-col max-w-full w-full gap-6">
      ...
      <Table<EmergencyType> data={processedEmergencies} columns={columns} />
    </div>
  );
}
