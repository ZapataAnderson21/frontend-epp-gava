...
export default function DocumentExpirations() {
  ...
  } = useFetch<ExpiringDocumentCategory[]>(
    `${expiringDocumentsApi}categories?includeDeleted=${includeDeleted}`,
    [includeDeleted],
  );
  const {
    data: documents,
  ...
  } = useFetch<ExpiringDocumentListResponse>(
    `${expiringDocumentsApi}?${listQuery}`,
    [listQuery],
  );
  ...
  >(
    selectedHistoryId
      ? `${expiringDocumentsApi}${selectedHistoryId}/history`
      : "",
    [selectedHistoryId],
  );
  ...
  const submitDocument = async (event: FormEvent) => {
    ...
    const ok = await runAction(() =>
      execute(
        editingDocumentId
          ? `${expiringDocumentsApi}${editingDocumentId}`
  ...
  const submitCategory = async (event: FormEvent) => {
    ...
    const payload = {
      ...
      notificationEmails: categoryForm.notificationEmails
        .split(/[;,\n]/)
    ...
    const ok = await runAction(() =>
      execute(
        editingCategoryId
          ? `${expiringDocumentsApi}categories/${editingCategoryId}`
          : `${expiringDocumentsApi}categories`,
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-xs text-gray-500">
                Registra la ubicación, controla las fechas y conserva el
                historial de cambios.
              </p>
            </div>
            {tab === "documents" ? (
              <Permission permission="documents.manage">
                <button
                  type="button"
                  onClick={() => {
                    resetDocumentForm();
                  ...
                  className="rounded-md bg-[#0047a3] px-5 py-2.5 font-bold text-white hover:bg-[#00377e]"
                >
                  Nuevo documento
                </button>
              </Permission>
          ...
          {tab === "documents" ? (
            <>
              ...
              <div className="grid gap-3 rounded-md border border-gray-200 bg-white p-4 md:grid-cols-4">
                <input
                  ...
                  placeholder="Buscar documento, código o referencia"
                  value={search}
                ...
                />
                <label className="flex items-center gap-2 rounded-md border border-gray-200 px-3 py-2 text-xs font-semibold">
                  <input
                    type="checkbox"
              ...
              {documentsLoading ? (
                ...
                <ErrorMessage errorMessage={documentsError} />
              ) : (
                <DocumentTable
                  ...
                  onArchive={(id) =>
                    runAction(() =>
                      execute(`${expiringDocumentsApi}${id}`, "DELETE"),
                    )
                  }
                  onRestore={(id) =>
                    runAction(() =>
                      execute(`${expiringDocumentsApi}${id}/restore`, "POST"),
                    )
                  }
                />
              )}
            ...
            <div className="grid gap-6 xl:grid-cols-[420px_minmax(0,1fr)]">
              ...
              <div className="flex flex-col gap-3">
                <label className="flex w-fit items-center gap-2 rounded-md border border-gray-200 bg-white px-3 py-2 text-xs font-semibold">
                  <input
                    type="checkbox"
                    checked={includeDeleted}
                    onChange={(event) =>
                ...
                {categoriesLoading ? (
                  ...
                  <ErrorMessage errorMessage={categoriesError} />
                ) : (
                  <div className="overflow-x-auto rounded-md border border-gray-200 bg-white">
                    <table className="w-full min-w-[700px] text-left text-xs">
                      ...
                      <tbody>
                        {(categories ?? []).map((category) => (
                          <tr
                            key={category.expiringDocumentCategoryId}
                          ...
                          >
                            <td className="px-4 py-3">
                              <p className="font-bold">
                                {category.name}{" "}
                                {category.deletedAt ? (
                                  <span className="ml-1 rounded bg-gray-100 px-2 py-0.5 text-2xs text-gray-500">
                                    Archivada
                                  </span>
                                ) : null}
                              </p>
                            ...
                            <td className="px-4 py-3 font-semibold">
                              {category.alertDaysFirst},{" "}
                              {category.alertDaysSecond} y{" "}
                              {category.alertDaysThird} días
                            </td>
                            <td className="px-4 py-3">
                              <p
                                className={
                                  category.emailNotificationsEnabled
                                    ? "text-emerald-700"
                                    : "text-gray-400"
                                }
                              >
                              ...
                              <p className="max-w-xs break-words text-2xs text-gray-500">
                                {category.notificationEmails.join(", ") ||
                                  "Sin destinatarios"}
                              </p>
                            </td>
                            <td className="px-4 py-3">
                                ...
                                {category.deletedAt ? (
                                  <ActionButton
                                    onClick={() =>
                                      runAction(() =>
                                        execute(
                                          `${expiringDocumentsApi}categories/${category.expiringDocumentCategoryId}/restore`,
                                          "POST",
                                        ),
                                      )
                                    }
                                  ...
                                  <>
                                    ...
                                    <ActionButton
                                      ...
                                      onClick={() =>
                                        runAction(() =>
                                          execute(
                                            `${expiringDocumentsApi}categories/${category.expiringDocumentCategoryId}`,
                                            "DELETE",
                                          ),
                                        )
                                      }
...
function DocumentForm({
  ...
  return (
    <form
      onSubmit={onSubmit}
      className="rounded-md border border-blue-100 bg-blue-50/40 p-5"
    >
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        ...
        <Field label="Código o número">
          <input
            value={form.documentCode}
            onChange={(e) => change("documentCode", e.target.value)}
            className="input"
          />
        </Field>
        <Field label="Tipo de referencia">
          <input
            required
            placeholder="Proyecto, trabajador, equipo..."
            value={form.referenceType}
            onChange={(e) => change("referenceType", e.target.value)}
            className="input"
          />
        </Field>
        <Field label="Especificar referencia">
          <input
            required
            value={form.referenceDescription}
            onChange={(e) => change("referenceDescription", e.target.value)}
            className="input"
          />
        </Field>
        <Field label="Espacio de almacenamiento">
          <input
            required
            placeholder="OneDrive, equipo local, espacio físico..."
            value={form.storageSpace}
            onChange={(e) => change("storageSpace", e.target.value)}
            className="input"
          />
        </Field>
        <Field label="Ruta de almacenamiento">
          <input
            placeholder="Ruta o enlace interno"
            value={form.storagePath}
            onChange={(e) => change("storagePath", e.target.value)}
            className="input"
          />
        </Field>
        <Field label="Descripción de ubicación">
          <input
            placeholder="Obligatoria si no hay ruta"
            value={form.storageDescription}
            onChange={(e) => change("storageDescription", e.target.value)}
            className="input"
          />
        </Field>
        ...
        <Field label="Fecha de vencimiento">
          <input
            ...
            onChange={(e) => change("expirationDate", e.target.value)}
            className="input"
          />
        </Field>
      ...
      <p className="mt-3 text-2xs font-semibold text-gray-500">
        Debes completar al menos la ruta o la descripción de ubicación.
      </p>
      <div className="mt-4 flex justify-end gap-2">
        <button
          type="button"
          onClick={onCancel}
          className="rounded-md border border-gray-300 px-4 py-2 font-bold"
        >
          Cancelar
        </button>
        <button
          disabled={saving}
          className="rounded-md bg-[#0047a3] px-4 py-2 font-bold text-white disabled:opacity-50"
        >
...
function CategoryForm({
  ...
  const change = (name: keyof typeof form, value: string | boolean) =>
    setForm((current) => ({ ...current, [name]: value }));
  return (
    <form
      onSubmit={onSubmit}
      className="h-fit rounded-md border border-gray-200 bg-white p-5 shadow-sm"
    >
      <h2 className="mb-4 text-lg font-extrabold">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="grid grid-cols-3 gap-2">
          <Field label="1.er aviso">
            <input
              required
              type="number"
              min="1"
              className="input"
              value={form.alertDaysFirst}
              onChange={(e) => change("alertDaysFirst", e.target.value)}
            />
          </Field>
          <Field label="2.º aviso">
            <input
              required
              type="number"
              min="1"
              className="input"
              value={form.alertDaysSecond}
              onChange={(e) => change("alertDaysSecond", e.target.value)}
            />
          </Field>
          <Field label="3.er aviso">
            <input
              required
              type="number"
              min="0"
              className="input"
              value={form.alertDaysThird}
              onChange={(e) => change("alertDaysThird", e.target.value)}
            />
          </Field>
        </div>
        <Field label="Destinatarios">
          <textarea
            className="input min-h-20"
            placeholder="correo1@gava.com, correo2@gava.com"
            value={form.notificationEmails}
            onChange={(e) => change("notificationEmails", e.target.value)}
          />
        </Field>
        <label className="flex items-center gap-2 text-xs font-semibold">
          <input
            ...
            onChange={(e) =>
              change("emailNotificationsEnabled", e.target.checked)
            }
          />{" "}
      ...
      <table className="w-full min-w-[1250px] text-left text-xs">
        ...
        <tbody>
          {documents.map((document) => (
            <tr
              key={document.expiringDocumentId}
              className={`border-t border-gray-100 ${document.deletedAt ? "bg-gray-50 opacity-70" : ""}`}
            >
              <td className="whitespace-nowrap px-4 py-3 font-bold">
                {formatDate(document.expirationDate)}
                <p className="text-2xs font-normal text-gray-500">
                  {document.daysRemaining >= 0
                    ? `${document.daysRemaining} días`
                    : `${Math.abs(document.daysRemaining)} días vencido`}
                </p>
              </td>
              <td className="px-4 py-3">
                <p className="font-bold">{document.title}</p>
                <p className="text-2xs text-gray-500">
                  {document.documentCode || "Sin código"}
                </p>
              </td>
              <td className="px-4 py-3">{document.category.name}</td>
              <td className="px-4 py-3">
                <p className="font-semibold">{document.referenceType}</p>
                <p className="max-w-xs text-2xs text-gray-500">
                  {document.referenceDescription}
                </p>
              </td>
              <td className="px-4 py-3">
                <p className="font-semibold">{document.storageSpace}</p>
                <p className="max-w-xs break-words text-2xs text-gray-500">
                  {document.storagePath || document.storageDescription}
                </p>
              </td>
              <td className="px-4 py-3">
                <span
                  className={`rounded-full px-3 py-1 text-2xs font-bold ${statusStyles[document.status]}`}
                >
                  {document.deletedAt
                    ? "Archivado"
                ...
                <div className="flex gap-2">
                  {!document.deletedAt ? (
                    <>
                      ...
                      <ActionButton
                        danger
                        onClick={() => onArchive(document.expiringDocumentId)}
                      >
                        Archivar
                      </ActionButton>
                    </>
                    ...
                    <ActionButton
                      onClick={() => onRestore(document.expiringDocumentId)}
                    >
                      Restaurar
                    </ActionButton>
                  )}
                  <ActionButton
                    onClick={() => onHistory(document.expiringDocumentId)}
                  >
                    Historial
                  </ActionButton>
                </div>
...
function HistoryList({ history }: { history: ExpiringDocumentHistory[] }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      {history.map((entry) => (
        <div
          key={entry.expiringDocumentHistoryId}
          className="rounded-md border border-gray-200 p-4"
        >
          <div className="flex flex-wrap justify-between gap-2">
            <p className="font-bold">{actions[entry.action]}</p>
            <p className="text-2xs text-gray-500">
          ...
          <details className="mt-2">
            <summary className="cursor-pointer text-2xs font-bold text-[#0047a3]">
              Ver versión guardada
            </summary>
            <pre className="mt-2 overflow-auto rounded bg-gray-50 p-3 text-2xs">
      ...
      {!history.length ? (
        <p className="text-center text-gray-500">Sin historial.</p>
      ) : null}
    </div>
  );
}
...
function PageTabButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`pb-2 text-base font-bold ${active ? "border-b-4 border-gray-900" : "text-gray-400"}`}
    >
      {children}
    </button>
  );
  ...
  return (
    ...
    >
      <button
        ...
        disabled={disabled}
        onClick={onClick}
        className={`rounded px-2 py-1 text-2xs font-bold disabled:cursor-not-allowed disabled:opacity-40 ${danger ? "bg-red-50 text-red-700" : "bg-blue-50 text-[#0047a3]"}`}
      >
        {children}
      </button>
    </Permission>
