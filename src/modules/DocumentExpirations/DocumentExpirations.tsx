      ...
      <Panel>
        <div className="flex w-full flex-col gap-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <h1 className="text-2xl font-extrabold">Vencimientos documentales</h1>
              <p className="mt-1 text-xs text-gray-500">Registra la ubicación, controla las fechas y conserva el historial de cambios.</p>
            </div>
            {tab === "documents" ? (
          ...
          {message ? <div className="rounded-md bg-emerald-50 p-3 text-xs font-semibold text-emerald-700">{message}</div> : null}
          {formError ? <div className="rounded-md bg-red-50 p-3 text-xs font-semibold text-red-700">{formError}</div> : null}
          ...
          {tab === "documents" ? (
            ...
            <div className="grid gap-6 xl:grid-cols-[420px_minmax(0,1fr)]">
              ...
              <div className="flex flex-col gap-3">
                <label className="flex w-fit items-center gap-2 rounded-md border border-gray-200 bg-white px-3 py-2 text-xs font-semibold"><input type="checkbox" checked={includeDeleted} onChange={(event) => setIncludeDeleted(event.target.checked)} /> Mostrar categorías archivadas</label>
              {categoriesLoading ? <LoadingSkeletonTable /> : categoriesError ? <ErrorMessage errorMessage={categoriesError} /> : (
                <div className="overflow-x-auto rounded-md border border-gray-200 bg-white">
                  <table className="w-full min-w-[700px] text-left text-xs">
                    <thead className="bg-gray-50 text-2xs uppercase text-gray-500"><tr><th className="px-4 py-3">Categoría</th><th className="px-4 py-3">Avisos</th><th className="px-4 py-3">Destinatarios</th><th className="px-4 py-3">Documentos</th><th className="px-4 py-3">Acciones</th></tr></thead>
                    <tbody>{(categories ?? []).map((category) => (
                      <tr key={category.expiringDocumentCategoryId} className="border-t border-gray-100">
                        <td className="px-4 py-3"><p className="font-bold">{category.name} {category.deletedAt ? <span className="ml-1 rounded bg-gray-100 px-2 py-0.5 text-2xs text-gray-500">Archivada</span> : null}</p><p className="text-2xs text-gray-500">{category.description || "Sin descripción"}</p></td>
                        <td className="px-4 py-3 font-semibold">{category.alertDaysFirst}, {category.alertDaysSecond} y {category.alertDaysThird} días</td>
                        <td className="px-4 py-3"><p className={category.emailNotificationsEnabled ? "text-emerald-700" : "text-gray-400"}>{category.emailNotificationsEnabled ? "Activos" : "Desactivados"}</p><p className="max-w-xs break-words text-2xs text-gray-500">{category.notificationEmails.join(", ") || "Sin destinatarios"}</p></td>
      ...
      {selectedHistoryId ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setSelectedHistoryId(null)}>
          <div className="max-h-[80vh] w-full max-w-3xl overflow-auto rounded-lg bg-white p-6 shadow-xl" onClick={(event) => event.stopPropagation()}>
            <div className="mb-4 flex items-center justify-between"><h2 className="text-lg font-extrabold">Historial del documento</h2><button type="button" onClick={() => setSelectedHistoryId(null)} className="text-xl">×</button></div>
...
function DocumentForm({ form, setForm, categories, editing, saving, onSubmit, onCancel }: {
  ...
  return (
    <form onSubmit={onSubmit} className="rounded-md border border-blue-100 bg-blue-50/40 p-5">
      ...
      <p className="mt-3 text-2xs font-semibold text-gray-500">Debes completar al menos la ruta o la descripción de ubicación.</p>
...
function CategoryForm({ form, setForm, editing, saving, onSubmit, onCancel }: {
  ...
  return (
    <form onSubmit={onSubmit} className="h-fit rounded-md border border-gray-200 bg-white p-5 shadow-sm">
      <h2 className="mb-4 text-lg font-extrabold">{editing ? "Editar categoría" : "Nueva categoría"}</h2>
      <div className="flex flex-col gap-3">
        ...
        <label className="flex items-center gap-2 text-xs font-semibold"><input type="checkbox" checked={form.emailNotificationsEnabled} onChange={(e) => change("emailNotificationsEnabled", e.target.checked)} /> Enviar avisos por correo</label>
      </div>
  ...
  return (
    <div className="overflow-x-auto rounded-md border border-gray-200 bg-white">
      <table className="w-full min-w-[1250px] text-left text-xs">
        <thead className="bg-gray-50 text-2xs uppercase text-gray-500"><tr><th className="px-4 py-3">Vencimiento</th><th className="px-4 py-3">Documento</th><th className="px-4 py-3">Categoría</th><th className="px-4 py-3">Referencia</th><th className="px-4 py-3">Almacenamiento</th><th className="px-4 py-3">Estado</th><th className="px-4 py-3">Acciones</th></tr></thead>
        <tbody>{documents.map((document) => (
          <tr key={document.expiringDocumentId} className={`border-t border-gray-100 ${document.deletedAt ? "bg-gray-50 opacity-70" : ""}`}>
            <td className="whitespace-nowrap px-4 py-3 font-bold">{formatDate(document.expirationDate)}<p className="text-2xs font-normal text-gray-500">{document.daysRemaining >= 0 ? `${document.daysRemaining} días` : `${Math.abs(document.daysRemaining)} días vencido`}</p></td>
            ...
            <td className="px-4 py-3"><p className="font-semibold">{document.referenceType}</p><p className="max-w-xs text-2xs text-gray-500">{document.referenceDescription}</p></td>
            <td className="px-4 py-3"><p className="font-semibold">{document.storageSpace}</p><p className="max-w-xs break-words text-2xs text-gray-500">{document.storagePath || document.storageDescription}</p></td>
            <td className="px-4 py-3"><span className={`rounded-full px-3 py-1 text-2xs font-bold ${statusStyles[document.status]}`}>{document.deletedAt ? "Archivado" : statusLabels[document.status]}</span></td>
...
function HistoryList({ history }: { history: ExpiringDocumentHistory[] }) {
  ...
  return <div className="flex flex-col gap-3">{history.map((entry) => <div key={entry.expiringDocumentHistoryId} className="rounded-md border border-gray-200 p-4"><div className="flex flex-wrap justify-between gap-2"><p className="font-bold">{actions[entry.action]}</p><p className="text-2xs text-gray-500">{new Date(entry.createdAt).toLocaleString("es-PE")}</p></div><p className="text-xs text-gray-600">Por {entry.changedBy.name} {entry.changedBy.lastName}</p><details className="mt-2"><summary className="cursor-pointer text-2xs font-bold text-[#0047a3]">Ver versión guardada</summary><pre className="mt-2 overflow-auto rounded bg-gray-50 p-3 text-2xs">{JSON.stringify(entry.snapshot, null, 2)}</pre></details></div>)}{!history.length ? <p className="text-center text-gray-500">Sin historial.</p> : null}</div>;
}

function Field({ label, children }: { label: string; children: React.ReactNode }) { return <label className="flex flex-col gap-1 text-xs font-semibold text-gray-700"><span>{label}</span>{children}</label>; }
function PageTabButton({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) { return <button type="button" onClick={onClick} className={`pb-2 text-base font-bold ${active ? "border-b-4 border-gray-900" : "text-gray-400"}`}>{children}</button>; }
function ActionButton({ onClick, children, danger = false, disabled = false }: { onClick: () => void; children: React.ReactNode; danger?: boolean; disabled?: boolean }) { return <button type="button" disabled={disabled} onClick={onClick} className={`rounded px-2 py-1 text-2xs font-bold disabled:cursor-not-allowed disabled:opacity-40 ${danger ? "bg-red-50 text-red-700" : "bg-blue-50 text-[#0047a3]"}`}>{children}</button>; }
