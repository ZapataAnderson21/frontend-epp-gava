...
export default function RowTable({ id, dateTime, status, user }: RowTableProps) {
  const statusClass = statusColorClasses[status] || statusColorClasses.default;

  const formattedDateTime = new Date(dateTime).toLocaleString("es-ES", {
    day: "2-digit",
    month: "2-digit",
    year: "numeric",
    hour: "2-digit",
  ...
  return (
    <a href={`/admin/requests/${id}`} className="w-full">
      <div className="flex flex-row items-center justify-between w-full p-4 border-b border-gray-200 
        ...
        <span className="flex items-start justify-start w-32">{formattedDateTime}</span>
