...
export default function Complaints() {
  ...
  const canViewAll = user?.userType === "SISTEMAS";
    ...
    <Panel>
      ...
      <div
        className={`grid w-full grid-cols-1 gap-8 ${
          !loadingCurrentUser && !canViewAll
            ? "xl:grid-cols-[minmax(0,620px)_1fr]"
            : ""
        }`}
      >
        {!loadingCurrentUser && !canViewAll && (
        ...
        <section className="flex min-w-0 flex-col gap-4">
          <h2 className="text-xl font-bold text-gray-800">
            {canViewAll ? "Todos los reclamos" : "Mis reclamos"}
          </h2>
                  ...
                  <div>
                    ...
                    {canViewAll && (
                      <p className="mt-1 text-xs text-gray-500">
                        Registrado por {item.user.name} {item.user.lastName} (
                        {item.user.email})
                      </p>
                    )}
                  </div>
